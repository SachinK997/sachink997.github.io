    ...
    function renderLogs(activities) {
        ...
        logsContainer.innerHTML = sorted.slice(0, 8).map(act => `
            <div style="display: flex; gap: 1rem; align-items: center; border-bottom: 1px solid rgba(255,255,255,0.05); padding-bottom: 0.75rem;">
                ...
                <div style="flex: 1;">
                    <h4 style="font-size: 0.9rem; margin-bottom: 0.1rem; color: var(--text-dark);">${act.name}</h4>
                    <p style="font-size: 0.75rem; color: var(--text-muted);">${act.date_key} • ${act.duration || 'N/A'}${act.intensity ? ' • Intensity ' + act.intensity : ''}</p>
                </div>
            </div>
        `).join('');
    }
